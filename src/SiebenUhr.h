...
void debug_color(CRGB color, int scale=1);
...
inline void debug_color(CRGB color, int scale) {
    const int SPACE = 20;
    if (scale == 1 && (color.r >= SPACE or color.g >= SPACE or color.g >= SPACE)) {
        ...
        scale = maximum(sr,maximum(sg,sb));
    }
...
inline void debug_value(String name, int value) {
    Serial.print(name);
    Serial.print("= ");
    Serial.println(value);
}

inline void debug_value(String name, String value) {
    Serial.print(name);
    Serial.print("= ");
    Serial.println(value);
}
...
inline void EEPROMWriteString(char eeprom_address,String data, int maxLength)
{
  if(data.equals(EEPROMReadString(eeprom_address, maxLength))) {
    // string already written, no need to mess with the EEPROM
    return;
  }
...
/* function to check if the supplied timezone can be found in the timezone.txt
   file on the SPIFF. */
